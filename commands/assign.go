...
func members(category string) ([]string, int) {
	switch category {
	case IOS:
		return []string{SUMIYOSHI, KATAGIRI, MORI, YABUSHITA, NAKAYAMA}, OK
	case ANDROID:
		return []string{SUMIYOSHI, KATAGIRI, YABUSHITA, NAKAYAMA, TAKADA}, OK
	case FRONT:
		return []string{UESHIMA, KATO, TOKUNAGA, KAMINAGA}, OK
	case BACK:
		return []string{SUMIYOSHI, YABUSHITA, NAKAYAMA, KATAGIRI, TOUYAMA, IDA}, OK
	case TECHPERM:
	...
	case ALL:
		return []string{SUMIYOSHI, UESHIMA, YABUSHITA, NAKAYAMA, KATAGIRI, KATO, TOKUNAGA, MORI, KAMINAGA, YANBE, TAKADA, TOUYAMA, IDA}, OK
	default:
		return nil, NG
	}
}
